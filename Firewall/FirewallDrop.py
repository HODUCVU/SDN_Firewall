        ...
        try:
            ...
            if(out_port != ofproto.OFPP_FLOOD) and (ethtype == ETH_TYPE_IP):
                ...
                # Check for UDP 
                    ...
                    if ipo.dst in self.udp_conn_track:
                        ...
                        for i in range(0, len(tmp)):
                            ...
                            if(tmp[i][0] == ipo.src):
                                xyz = tmp[i]
                                # <here>
                                if (int(xyz[1]) == udpo.dst_port) and (int(xyz[2]) == udpo.src_port) and (xyz[3] == "DROPPED"):
                                    flag3 = 1 
                                    self.logger.info("%s -> %s : UDP PACKET DROP" % (ipo.src, ipo.dst))
                                    actions_default = action_drop
                                    del tmp[i]
                                    self.logger.info("\n%s -> %s src_port= %s, dst_port= %s, state= BLOCKED\n" %(ipo.src, ipo.dst,
                            ...
                            if temp[i][0] == ipo.dst:
                                ...
                                if (xyz[1] == 'UDP') and (int(xyz[2]) == udpo.src_port) and (int(xyz[3]) == udpo.dst_port) and (xyz[5] == 'DROP'): 
                                    ...
                                    self.udp_conn_track = self.track.conn_track_dict(self.udp_conn_track, ipo.src, ipo.dst, udpo.src_port, 
                                                                                     udpo.dst_port, "DROPPED", 1)
                                    self.logger.info("%s -> %s : UDP PACKET BLOCKED" % (ipo.src, ipo.dst))
                    ...
                    if flag3 == 0:
                        actions_default = action_fwd_to_out_port
                        # self.logger.info("%s -> %s : UDP ALLOW" % (ipo.src, ipo.dst))
